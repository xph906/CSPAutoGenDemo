...
var CSPAutoGenTemplateFunctions = function(){
	 ...
	 */
	var extractEvalLikeScripts = function(gasts, evalArr){
		for(var i in gasts){
			estraverse.traverse(gasts[i], {
				enter: function(node){
					if(toolSet.isEvalCallNode(node)){
						if(node.arguments.length > 0)
							evalArr.push(node.arguments[0].value.toString());
					}
	...
	var genSymTemplateStrArrayObjectHelperNodeHandler = function(node, elems){
	  ...
	  else if(toolSet.isObjectNode("object") ) {
	...
	var genSymTemplateNode = function(template, hash, params, templates, returnIdentifier){
	  ...
	  //console.log("Start generate SYM for "+escodegen.generate(template));
	  estraverse.traverse(template, {
	    enter: function(node){
	    	//if(node.CSPTag === "object"){
	    	//	console.log("ENTER OBJECT: "+escodegen.generate(node));
	    	//}
	      ...
	      if(subtype === "string"){
	        	...
	        	if(!evalTemplate){
	        		console.log("error: cannot find eval template.");
	        		for(var h in templates)
	        			console.log("imported templates: "+h)
	        	}
	        	else{
	        		var evalFunArgs = [];
		        	var hash = toolSet.generateGASTHash(evalTemplate);
	     	...
	     	//console.log("IN OBJECT: "+escodegen.generate(node));
	      ...
	      else if(node.CSPTag === "Program"){
	      	programNode = node;
	      }
	      
	      //don't go any further for object and array nodes.
	      if(toolSet.isObjectNode(node) || toolSet.isArrayNode(node) ){
	        ...
	        //console.log("SKIP [LEAVE ENTER]: "+escodegen.generate(node));
	        return estraverse.VisitorOption.Skip;
	      }
	    ...
	    leave: function (node, parent) {
	      if(toolSet.isLiteralNode(node) && !node.regex){
	      	//console.log("LEAVE NODE: "+escodegen.generate(node));
	        var found = false;
	        for(var k in parent){
	          if(parent[k] === node){    
	            if (node.hasOwnProperty('newNode')){
...
module.exports = {
	...
	matchScript : templateFunctions.matchScript,
	matchScriptValues : templateFunctions.matchScriptValues
};